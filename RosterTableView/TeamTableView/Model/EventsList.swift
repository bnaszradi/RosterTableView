...
class EventsList {
  ...
  //  func eventsQuery(tName: String) -> (resultsNameArray: Array<String>, resultsDateArray: Array<Date>) {
    ...
    func eventDateQuery(tName: String, eName: String, completion: @escaping (QEvents)->Void)  {
        
        var resultsNameArray = [] as Array<String>
       
        var resultsDateArray = [] as Array<Date>
        
       print("tName in eventDateQuery: ", tName)
         
         let eventsPredicate = NSPredicate(format: "team == %@ AND eventName == %@", tName, eName)
        // print("teamPredicate: ", teamPredicate)
